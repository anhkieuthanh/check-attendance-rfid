...
/**
 * Dumps card Uid about the selected PICC to Serial.
 */
void MFRC522::PICC_DumpDetailsToSerialUid(Uid *uid	///< Pointer to Uid struct returned from a successful PICC_Select().
									) {
	// UID
	Serial.print(F("Card UID:"));
	for (byte i = 0; i < uid->size; i++) {
	...
	Serial.println();
} // End PICC_DumpDetailsToSerialUid()
